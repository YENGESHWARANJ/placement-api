...
exports.getMyInterviews = exports.submitFeedback = exports.scheduleInterview = exports.evaluateResponse = exports.generateQuestions = void 0;
...
const interview_session_model_1 = __importDefault(require("./interview-session.model"));
...
const generateQuestions = async (req, res) => {
    try {
        ...
        if (skills.some((s) => (s.toLowerCase().includes("javascript") || s.toLowerCase().includes("node")))) {
            techQuestions.push("Explain the event loop in Node.js.", "What is the difference between '==' and '==='?");
        }
        if (skills.some((s) => (s.toLowerCase().includes("sql") || s.toLowerCase().includes("database")))) {
...
const scheduleInterview = async (req, res) => {
    try {
        ...
        const interview = await interview_session_model_1.default.create({
            applicationId,
        ...
        await notice_model_1.default.create({
            ...
            content: `You have a new interview scheduled for ${application.jobId.title} at ${new Date(scheduledAt).toLocaleString()}`,
            type: "Student",
            priority: "High",
            createdBy: userId,
...
const submitFeedback = async (req, res) => {
    try {
        ...
        const interview = await interview_session_model_1.default.findById(id).populate("studentId");
        if (!interview)
...
const getMyInterviews = async (req, res) => {
    try {
        ...
        const interviews = await interview_session_model_1.default.find(query)
